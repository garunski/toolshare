...
### 1. Create Initial Schema File
- [ ] Create: `supabase/migrations/20240101000001_add_user_roles_and_permissions.sql`
- [ ] This extends the existing initial schema with RBAC functionality

### 2. Complete SQL Schema Template
...
-- Roles: Only admins can manage, everyone can view
...
-- Permissions: Only admins can manage, everyone can view  
...
-- Role Permissions: Only admins can manage, everyone can view
...
- [ ] Add the schema to existing initial migration or create new migration file
- [ ] Run: `task database:db-migrate` to apply schema
- [ ] Verify schema applied successfully
- [ ] Check tables created: `roles`, `permissions`, `role_permissions`, `user_roles`

### 4. Generate Types
- [ ] Run: `task database:db-types`
- [ ] Verify new types appear in `src/types/supabase.ts`
- [ ] Check for `Role`, `Permission`, `UserRole`, `RolePermission` types

### 5. Validation Queries
Test the schema with these queries in Supabase SQL Editor:
...
SELECT public.user_has_permission(auth.uid(), 'manage_users');
SELECT * FROM public.get_user_roles(auth.uid());
```

## Success Criteria
- [ ] Initial schema file created and applied successfully
- [ ] All 4 RBAC tables exist with proper relationships
