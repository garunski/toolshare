...
interface FormState {
  ...
  getFormProgress: (
    formId: string,
  ) => { totalFields: number; completedFields: number; lastSaved: Date } | null;
  ...
  persist(
    (set, get) => ({
      ...
      getValidationCache: (fieldKey: string) => {
        ...
        const minutesDiff =
          (new Date().getTime() - cached.lastValidated.getTime()) / (1000 * 60);
        if (minutesDiff > 5) {
      ...
      switch (attribute.data_type) {
        ...
        case "multi_select":
          if (
            ...
            Array.isArray(value) &&
            value.length < rules.min_selections
          ) {
            errors.push(
          ...
          if (
            ...
            Array.isArray(value) &&
            value.length > rules.max_selections
          ) {
            errors.push(
