...
const SmartDefaultsContext = createContext<SmartDefaultsContextType | null>(
  null,
);
  ...
  const contextValue = useMemo(() => {
    const getDefaults = (
      ...
      if (categoryId && categoryDefaults[categoryId]) {
        Object.entries(categoryDefaults[categoryId]).forEach(
          ([field, value]) => {
            defaults.push({
              field,
              value,
    ...
    const getFieldHelp = (
      ...
      const helpTexts: Record<string, string> = {
        ...
        description:
          "Provide detailed information about condition, features, and usage",
...
export function useSmartDefaults() {
  ...
  if (!context) {
    throw new Error(
      "useSmartDefaults must be used within a SmartDefaultsProvider",
    );
  }
  return context;
}
