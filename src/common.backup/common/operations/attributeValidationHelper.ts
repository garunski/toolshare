export class AttributeValidationHelper {
  /**
   * Create validation function from rules
   */
  static createValidator(rules: any): ((value: any) => boolean) | undefined {
  ...
  static validateData(
    ...
    Object.entries(mappedData).forEach(([fieldName, value]) => {
      ...
      if (attr && attr.validationRules) {
        const validator = this.createValidator(attr.validationRules);
